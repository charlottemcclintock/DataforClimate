...
# map$happening <- cut(map$happening, breaks = seq(45,90,5))

# BELIEVE CLIMATE CHANGE IS HAPPENING
p <- map %>%
  group_by(group) %>%
  plot_mapbox(x = ~long, y = ~lat, color = ~as.factor(happening), 
              colors = c('yellow','blue'),
              text = ~paste(subregion, ":", happening), hoverinfo = 'text', 
              showlegend = FALSE) %>%
  add_polygons(
    line = list(width = 0.4)
  ) %>%
...
p <- map %>%
  ...
  plot_mapbox(x = ~long, y = ~lat, color = ~as.factor(round(worried)), 
              colors = c('yellow','blue'),
              text = ~paste(subregion, ":", worried), hoverinfo = 'text', 
              showlegend = FALSE) %>%
  add_polygons(
    line = list(width = 0.4)
  ) %>%
...
p <- map %>%
  ...
  plot_mapbox(x = ~long, y = ~lat, color = ~as.factor(round(personal)), 
              colors = c('yellow','blue'),
              text = ~paste(subregion, personal), hoverinfo = 'text', 
              showlegend = FALSE) %>%
  add_polygons(
    line = list(width = 0.4)
  ) %>%
...
# 
map$diff_map <- cut(map$diff, 4)

# HAPPENING BUT NOT WORRIED
p <- map %>%
  group_by(group) %>%
  plot_mapbox(x = ~long, y = ~lat, color = ~as.factor((diff_map)), 
              colors = c("#156d03", "#5c944a", "#97bd88", "#d1e6c8"),
